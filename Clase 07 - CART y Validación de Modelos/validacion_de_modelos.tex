...
En machine learning, distinguimos principalmente entre dos tipos de problemas:
\begin{itemize}
    \item \textbf{Regresión}: Cuando queremos predecir un valor numérico continuo (ej: precio de una casa, temperatura, salario)
    \item \textbf{Clasificación}: Cuando queremos predecir una categoría o clase (ej: spam/no spam, enfermedad/sano, compra/no compra)
\end{itemize}

Cada tipo de problema requiere métricas de validación específicas y diferentes enfoques para evaluar el rendimiento del modelo.

\section{Validación para Regresión}

En problemas de regresión, nuestro objetivo es predecir un valor numérico continuo. Las métricas de validación para regresión se enfocan en medir qué tan cerca están nuestras predicciones de los valores reales.

\subsection{Error Cuadrático Medio (MSE)}

El Error Cuadrático Medio (Mean Squared Error) es una de las métricas más utilizadas en regresión. Penaliza más los errores grandes debido a la función cuadrática:
...
El RMSE es simplemente la raíz cuadrada del MSE, lo que nos da una métrica en las mismas unidades que la variable objetivo:
...
El Error Absoluto Medio (Mean Absolute Error) es menos sensible a valores atípicos que el MSE, ya que no eleva al cuadrado los errores:

\[
\text{MAE} = \frac{1}{n} \sum_{i=1}^{n} |y_i - \hat{y}_i|
\]

\subsection{Coeficiente de Determinación (R²)}

El R² mide qué proporción de la varianza en la variable dependiente es explicada por el modelo. Su valor va de 0 a 1:
...
\begin{itemize}
    \item $R^2 = 1$: El modelo explica perfectamente la varianza
    \item $R^2 = 0$: El modelo no es mejor que predecir la media
    \item $R^2 < 0$: El modelo es peor que predecir la media
\end{itemize}

\subsection{Error Porcentual Absoluto Medio (MAPE)}

El MAPE expresa el error como porcentaje del valor real, lo que facilita la interpretación:

\[
\text{MAPE} = \frac{100\%}{n} \sum_{i=1}^{n} \left|\frac{y_i - \hat{y}_i}{y_i}\right|
\]

\textbf{Problema con valores cero:} Cuando $y_i = 0$, la fórmula tiene una división por cero que causa error. Soluciones comunes:
\begin{itemize}
    \item Reemplazar ceros por un valor muy pequeño (ej: 0.0000001)
    \item Usar un porcentaje del valor promedio (ej: 1\% de la media)
    \item Filtrar los valores cero antes del cálculo
    \item Usar MAPE simétrico (sMAPE): $\text{sMAPE} = \frac{100\%}{n} \sum_{i=1}^{n} \frac{2|y_i - \hat{y}_i|}{|y_i| + |\hat{y}_i|}$
\end{itemize}

\subsection{Ejemplo Práctico de Regresión}

Consideremos un modelo que predice el precio de casas. Tenemos los siguientes datos:
...
\begin{align*}
\text{MSE} &= \frac{1}{5}[(30-28)^2 + (45-47)^2 + (20-19)^2 + (60-62)^2 + (35-33)^2] \\
&= \frac{1}{5}[4 + 4 + 1 + 4 + 4] \\
&= 18 \\
\text{RMSE} &= \sqrt{18} = 4.24 \\
\text{MAE} &= \frac{1}{5}[20 + 20 + 10 + 20 + 20] = 18000 \\
\text{MAPE} &= \frac{100\%}{5}[\frac{20}{30} + \frac{20}{45} + \frac{10}{20} + \frac{20}{60} + \frac{20}{35}] \\
&= 4.95\%
\end{align*}

\section{Validación para Clasificación}

En problemas de clasificación, nuestro objetivo es predecir una categoría o clase. Las métricas de validación para clasificación se enfocan en medir la precisión de las predicciones categóricas.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/curva-roc.png}
\end{figure}
...
\subsection{Casos de Uso y Recomendaciones}
...
\subsection{Ejemplo Práctico de Clasificación}
...
\subsubsection{Elección del número de folds (k)}
...
\subsubsection{Ordenamiento y mezcla de datos}
\begin{itemize}
