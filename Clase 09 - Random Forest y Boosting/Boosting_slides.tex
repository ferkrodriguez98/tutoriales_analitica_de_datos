...
\begin{frame}{AdaBoost (1995)}
    ...
    \begin{itemize}
        \item Todas las observaciones inician con el mismo peso
        \item En cada iteración:
        \begin{itemize}
            \item<2-> Se entrena un modelo débil
            \item<3-> Se identifican errores
            \item<4-> Se aumenta el peso de errores
            \item<5-> Se disminuye el peso de aciertos
        \end{itemize}
        \item Predicción final por promedio ponderado
    \end{itemize}
\end{frame}
...
\begin{frame}{Gradient Boosting - Características}
    ...
    \begin{itemize}
        \item Utiliza el gradiente del error para identificar la dirección de máximo descenso
        \item Aplicable a cualquier función de pérdida diferenciable
        \item Más flexible que AdaBoost al no limitarse a clasificación binaria
        \item Mayor control sobre el proceso de aprendizaje
    \end{itemize}
\end{frame}
...
\begin{frame}{Gradient Boosting - Parámetros}
    ...
    \begin{itemize}
        \item n\_estimators: Número de árboles
        \item learning\_rate: Tasa de aprendizaje
        \item max\_depth: Profundidad máxima
        \item subsample: Fracción de datos por árbol
    \end{itemize}
\end{frame}
...
\begin{frame}{XGBoost - Optimizaciones}
    ...
    \begin{itemize}
        \item Paralelización y optimización de memoria: Procesamiento más rápido usando múltiples núcleos
        \item Estructuras de datos especializadas: Formatos optimizados para mejor rendimiento
        \item Regularización L1 y L2 incorporada: Previene overfitting automáticamente
        \item Manejo automático de valores faltantes: Procesa datos incompletos sin preprocesamiento
    \end{itemize}
\end{frame}
...
\begin{frame}{XGBoost - Características Avanzadas}
    ...
    \begin{itemize}
        \item Early stopping inteligente: Monitorea el rendimiento y detiene el entrenamiento cuando no hay mejora
        \item Feature Importance: Proporciona métricas precisas sobre la contribución de cada variable
        \item Procesamiento optimizado de datos dispersos: Utiliza estructuras especializadas para matrices sparse
        \item Entrenamiento distribuido: Permite escalar el entrenamiento a múltiples núcleos de procesamiento
    \end{itemize}
\end{frame}
